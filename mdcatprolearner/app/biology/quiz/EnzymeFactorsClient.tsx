// app/blood-vessels EnzymeClient.tsx
"use client"
...
import { enzymeRateFactorsQuestions } from "../data/enzymeRateFactorsQuestions"; // <-- adjust path if needed

type AnswerMap = Record<string, number | null>;

export default function EnzymeFactorClient() {
  ...
  const total = enzymeRateFactorsQuestions.length;
  const question = enzymeRateFactorsQuestions[index];
  ...
  const handleSelect = (qId: string, chosenIndex: number) => {
    ...
    const q = enzymeRateFactorsQuestions.find((qq) => qq.id === qId);
    if (!q) return;
  ...
  const computeScore = () => {
    let c = 0;
    for (const q of enzymeRateFactorsQuestions) {
  ...
  const optionAriaLabel = (qId: string, idx: number, text: string) => {
    ...
    const isCorrect = enzymeRateFactorsQuestions.find((qq) => qq.id === qId)?.correctAnswer === idx;
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name:  "Enzyme Rate Factors Quiz",
    description: "Practice MDCAT questions on factors affecting enzyme reaction rates with multiple choice questions and detailed explanations.",
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!started && !showReview && score == null ? (
          <section className="bg-white rounded-2xl shadow p-6 mt-8">
            <div className="text-center">
              <h1 className="text-2xl font-bold mb-2">Enzyme — Practice MCQs</h1>
        ...
        {showReview && (
          <section className="bg-white rounded-xl shadow mt-6 p-5">
            ...
            <div className="space-y-3">
              {enzymeRateFactorsQuestions.map((q, idx) => {
