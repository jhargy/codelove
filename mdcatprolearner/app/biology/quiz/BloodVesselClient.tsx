// app/blood-vessels/bloodVesselsClient.tsx
"use client";
...
import { bloodVesselsQuestions } from "../data/bloodVesselsQuestions"; // <-- adjust path if needed

type AnswerMap = Record<string, number | null>;

export default function BloodVesselsClient() {
  ...
  const total = bloodVesselsQuestions.length;
  const question = bloodVesselsQuestions[index];
  ...
  const handleSelect = (qId: string, chosenIndex: number) => {
    ...
    const q = bloodVesselsQuestions.find((qq) => qq.id === qId);
    if (!q) return;
  ...
  const computeScore = () => {
    let c = 0;
    for (const q of bloodVesselsQuestions) {
  ...
  const optionAriaLabel = (qId: string, idx: number, text: string) => {
    ...
    const isCorrect = bloodVesselsQuestions.find((qq) => qq.id === qId)?.correctAnswer === idx;
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name: "Blood Vessels Quiz",
    description: "MCQs covering arteries, veins and capillaries — practice for medical tests.",
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!started && !showReview && score == null ? (
          <section className="bg-white rounded-2xl shadow p-6 mt-8">
            <div className="text-center">
              <h1 className="text-2xl font-bold mb-2">Blood Vessels — Practice MCQs</h1>
        ...
        {showReview && (
          <section className="bg-white rounded-xl shadow mt-6 p-5">
            ...
            <div className="space-y-3">
              {bloodVesselsQuestions.map((q, idx) => {
