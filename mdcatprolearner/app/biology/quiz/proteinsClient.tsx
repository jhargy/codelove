// app/blood-vessels/ProteinsClient.tsx
"use client"
...
import { proteinsQuestions } from "../data/proteinsQuestions"; // <-- adjust path if needed

type AnswerMap = Record<string, number | null>;

export default function ProteinsClient() {
  ...
  const total = proteinsQuestions.length;
  const question = proteinsQuestions[index];
  ...
  const handleSelect = (qId: string, chosenIndex: number) => {
    ...
    const q = proteinsQuestions.find((qq) => qq.id === qId);
    if (!q) return;
  ...
  const computeScore = () => {
    let c = 0;
    for (const q of proteinsQuestions) {
  ...
  const optionAriaLabel = (qId: string, idx: number, text: string) => {
    ...
    const isCorrect = proteinsQuestions.find((qq) => qq.id === qId)?.correctAnswer === idx;
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name: "Proteins Quiz",
    description: "Practice MDCAT Proteins questions with MCQs. Test your knowledge of amino acids, protein structure, enzymes, and their biological functions with explanations and instant feedback.",
        ...
        {!started && !showReview && score == null ? (
          <section className="bg-white rounded-2xl shadow p-6 mt-8">
            <div className="text-center">
              <h1 className="text-2xl font-bold mb-2">Proteins — Practice MCQs</h1>
