...
import { muscleContractionQuestions } from "../data/muscleContractionQuestions"; // <-- adjust path if needed
...
export default function ProteinsClient() {
  ...
  const total = muscleContractionQuestions.length;
  const question = muscleContractionQuestions[index];
  ...
  const handleSelect = (qId: string, chosenIndex: number) => {
    ...
    const q = muscleContractionQuestions.find((qq) => qq.id === qId);
    if (!q) return;
  ...
  const computeScore = () => {
    let c = 0;
    for (const q of muscleContractionQuestions) {
  ...
  const optionAriaLabel = (qId: string, idx: number, text: string) => {
    ...
    const isCorrect = muscleContractionQuestions.find((qq) => qq.id === qId)?.correctAnswer === idx;
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name: "Muscle Contraction Quiz",
            description:
              "Practice MDCAT-level muscle contraction questions with explanations.",
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {showReview && (
          <section className="bg-white rounded-xl shadow mt-6 p-5">
            ...
            <div className="space-y-3">
              {muscleContractionQuestions.map((q, idx) => {
