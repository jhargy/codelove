...
import { enzymeQuestions } from "../data/enzymeQuestions"; // <-- adjust path if needed

type AnswerMap = Record<string, number | null>;

export default function EnzymeClient() {
  ...
  const total = enzymeQuestions.length;
  const question = enzymeQuestions[index];
  ...
  const handleSelect = (qId: string, chosenIndex: number) => {
    ...
    const q = enzymeQuestions.find((qq) => qq.id === qId);
    if (!q) return;
  ...
  const computeScore = () => {
    let c = 0;
    for (const q of enzymeQuestions) {
  ...
  const optionAriaLabel = (qId: string, idx: number, text: string) => {
    ...
    const isCorrect = enzymeQuestions.find((qq) => qq.id === qId)?.correctAnswer === idx;
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name:  "Enzyme Quiz",
    description: "Practice MDCAT Enzyme questions with MCQs. Test your knowledge of enzyme structure, function, kinetics, and regulation with explanations and instant feedback.",
