// app/blood-vessels/prokaryoticAndEukaryoticClient.tsx
"use client";
...
import { prokaryoticEukaryoticQuestions } from "../data/prokaryoticEukaryoticQuestions"; // <-- adjust path if needed

type AnswerMap = Record<string, number | null>;


export default function prokaryoticAndEukaryoticClient() {
  ...
  const total = prokaryoticEukaryoticQuestions.length;
  const question = prokaryoticEukaryoticQuestions[index];
  ...
  const handleSelect = (qId: string, chosenIndex: number) => {
    ...
    const q = prokaryoticEukaryoticQuestions.find((qq) => qq.id === qId);
    if (!q) return;
  ...
  const computeScore = () => {
    let c = 0;
    for (const q of prokaryoticEukaryoticQuestions) {
  ...
  const optionAriaLabel = (qId: string, idx: number, text: string) => {
    ...
    const isCorrect = prokaryoticEukaryoticQuestions.find((qq) => qq.id === qId)?.correctAnswer === idx;
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name: "prokaryotic And Eukaryotic Quiz",
    description: "Practice MDCAT prokaryoticAndEukaryotic questions with and instant feedback.",
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!started && !showReview && score == null ? (
          <section className="bg-white rounded-2xl shadow p-6 mt-8">
            <div className="text-center">
              <h1 className="text-2xl font-bold mb-2">prokaryoticAndEukaryotic — Practice MCQs</h1>
        ...
        {showReview && (
          <section className="bg-white rounded-xl shadow mt-6 p-5">
            ...
            <div className="space-y-3">
              {prokaryoticEukaryoticQuestions.map((q, idx) => {
